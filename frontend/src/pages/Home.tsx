...
const navItems = [
  ...
  { en: "Expertise", hi: "विशेषज्ञता", href: "expertise" },
  { en: "Pricing", hi: "शुल्क", href: "pricing" },
  { en: "How it works", hi: "प्रक्रिया", href: "process" },
  { en: "Contact", hi: "संपर्क", href: "contact" },
];

const uiCopy = {
  en: { top: "Round-the-clock legal support / India + international matters / Delivery within 24 hours", book: "Book consultation", heroEyebrow: "A global platform for legal solutions", heroFirst: "Trusted legal solutions.", heroSecond: "Anytime. Anywhere.", heroBody: "Justice, professionalism, and client trust are the foundations of everything we do. Expert legal guidance across borders — under one roof.", explore: "Explore services", about: "Who we are", pricing: "Transparent & fair", booking: "Schedule instantly", process: "The process", resources: "Legal knowledge center", contact: "Get in touch", submit: "Send consultation request" },
  hi: { top: "चौबीसों घंटे कानूनी सहायता / भारत और अंतरराष्ट्रीय मामले / 24 घंटे में सेवा", book: "परामर्श बुक करें", heroEyebrow: "एक छत के नीचे वैश्विक कानूनी समाधान", heroFirst: "विश्वसनीय कानूनी समाधान।", heroSecond: "कभी भी। कहीं भी।", heroBody: "न्याय, पेशेवर सेवा और ग्राहक का विश्वास हमारे हर कार्य की नींव हैं। सीमाओं के पार विशेषज्ञ कानूनी मार्गदर्शन।", explore: "सेवाएं देखें", about: "हमारे बारे में", pricing: "पारदर्शी और उचित", booking: "तुरंत समय तय करें", process: "हमारी प्रक्रिया", resources: "कानूनी ज्ञान केंद्र", contact: "संपर्क करें", submit: "परामर्श अनुरोध भेजें" },
} as const;
...
export default function Home() {
  ...
  const [trackForm, setTrackForm] = useState<BookingTrackRequest>({ id: "", mobile: "" });
  ...
  const trackingMutation = useMutation({
    mutationFn: (payload: BookingTrackRequest) => apiPost<BookingTracking>("/bookings/track", payload),
    onError: () => toast.error("Booking not found", { description: "Check the reference and mobile number, then try again." }),
  });
  ...
  const shareArticle = async () => {
    ...
    const payload = { title: `${selectedArticle.title} | SpLegalMart`, text: selectedArticle.summary, url };
    if (navigator.share) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f8fafc] text-[#0f172a]" id="top" style={{ "--global-image": `url("${siteContent.global_image}")`, "--trust-image": `url("${siteContent.trust_image}")` } as CSSProperties}>
      <div className="bg-[#0f172a] px-5 py-2 text-center text-[10px] font-medium uppercase tracking-[0.18em] text-slate-300" data-testid="top-service-ribbon">{t.top}</div>
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl" data-testid="site-header">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary navigation" data-testid="desktop-navigation">
            {navItems.map((item) => <a className="text-xs font-medium uppercase tracking-[0.14em] text-slate-600 transition-colors hover:text-[#b45309]" href={`#${item.href}`} key={item.href} data-testid={`nav-link-${item.href}`}>{item[language]}</a>)}
          </nav>
          <div className="hidden items-center gap-5 lg:flex">
            <button type="button" onClick={toggleLanguage} className="inline-flex items-center gap-2 text-xs font-medium uppercase tracking-[0.12em] text-slate-500 transition-colors hover:text-[#b45309]" aria-label="Switch language" data-testid="language-toggle-button"><Languages className="size-4" />{language === "en" ? "हिंदी" : "EN"}</button>
            <a href={phoneHref} className="flex items-center gap-2 text-xs font-medium text-slate-600 transition-colors hover:text-[#b45309]" data-testid="header-phone-link"><Phone className="size-3.5" />{siteContent.phone.split("/")[0]}</a>
            <a href="#booking" className="group inline-flex items-center gap-2 bg-[#b45309] px-4 py-3 text-xs font-medium uppercase tracking-[0.13em] text-white transition-[transform,box-shadow,background-color] duration-200 hover:-translate-y-1 hover:bg-[#92400e] hover:shadow-lg" data-testid="header-book-consultation-button">{t.book} <ArrowUpRight className="size-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></a>
          </div>
        ...
        {mobileOpen && <nav className="border-t border-slate-200 bg-white px-5 py-4 lg:hidden" aria-label="Mobile navigation" data-testid="mobile-navigation">{navItems.map((item) => <a className="block border-b border-slate-100 py-3 text-xs font-medium uppercase tracking-[0.14em] text-slate-600" href={`#${item.href}`} key={item.href} onClick={() => setMobileOpen(false)} data-testid={`mobile-nav-link-${item.href}`}>{item[language]}</a>)}<button type="button" onClick={toggleLanguage} className="mt-3 flex w-full items-center justify-center gap-2 border border-slate-200 px-4 py-3 text-xs uppercase tracking-[0.13em] text-slate-600" data-testid="mobile-language-toggle-button"><Languages className="size-4" />{language === "en" ? "हिंदी" : "English"}</button><a href="#booking" className="mt-3 block bg-[#b45309] px-4 py-3 text-center text-xs font-medium uppercase tracking-[0.13em] text-white" onClick={() => setMobileOpen(false)} data-testid="mobile-book-consultation-button">{t.book}</a></nav>}
      </header>
      ...
      <main>
        <section className="noise-dark relative min-h-[720px] overflow-hidden bg-[#020617]" aria-labelledby="hero-title" data-testid="hero-section">
          <img className="absolute inset-0 size-full object-cover opacity-70" src={siteContent.hero_image} alt="Global legal services, justice scales and city skyline" data-testid="hero-background-image" />
          ...
          <div className="relative mx-auto grid min-h-[650px] max-w-7xl items-end gap-10 px-5 pb-20 pt-24 lg:grid-cols-12 lg:px-8 lg:pb-28">
            <div className="animate-fade-up lg:col-span-8" data-testid="hero-copy">
              ...
              <div className="mt-10 flex flex-wrap items-center gap-4">
                <a href="#booking" className="group inline-flex items-center gap-3 bg-[#b45309] px-6 py-4 text-xs font-medium uppercase tracking-[0.16em] text-white transition-[transform,box-shadow,background-color] duration-200 hover:-translate-y-1 hover:bg-[#d97706] hover:shadow-xl" data-testid="hero-book-consultation-button">{t.book} <ArrowUpRight className="size-4 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></a>
                <a href="#expertise" className="group inline-flex items-center gap-2 px-3 py-4 text-xs font-medium uppercase tracking-[0.16em] text-white transition-colors hover:text-[#d7a652]" data-testid="hero-explore-services-link">{t.explore} <ChevronRight className="size-4 transition-transform group-hover:translate-x-1" /></a>
              </div>
            </div>
        ...
        <section id="about" className="mx-auto max-w-7xl px-5 py-24 lg:px-8 lg:py-32" data-testid="about-section">
          <div className="grid gap-14 lg:grid-cols-12 lg:gap-20">
            <div className="lg:col-span-5"><SectionLabel testId="about-eyebrow">{t.about}</SectionLabel><h2 className="font-heading text-4xl leading-tight tracking-tight sm:text-5xl" data-testid="about-title">A steadier way through complex legal matters.</h2><p className="mt-7 whitespace-pre-line text-base leading-8 text-slate-600" data-testid="about-description">{siteContent.about}</p><p className="mt-5 text-base leading-8 text-slate-600" data-testid="about-description-secondary">We combine legal expertise, thorough research, strategic planning, and professional dedication to bridge complex multi-jurisdictional needs with efficient business operations.</p><a href="#booking" className="mt-8 inline-flex items-center gap-2 border-b border-[#b45309] pb-2 text-xs font-medium uppercase tracking-[0.15em] text-[#92400e] transition-[gap] hover:gap-4" data-testid="about-book-link">Speak with our legal desk <ArrowUpRight className="size-4" /></a><div className="mt-10 flex items-center gap-4 border-t border-slate-200 pt-6" data-testid="leadership-profile"><img src={siteContent.leader_image} alt={siteContent.leader_name} className="size-20 rounded-full border-2 border-[#d7a652] object-cover" data-testid="leadership-photo" /><div><p className="font-heading text-xl" data-testid="leadership-name">{siteContent.leader_name}</p><p className="mt-1 text-xs uppercase tracking-[0.12em] text-[#b45309]" data-testid="leadership-title">{siteContent.leader_title}</p></div></div></div>
        ...
        <section id="resources" className="bg-[#eef2f6] px-5 py-24 lg:px-8 lg:py-28" data-testid="resources-section">
          <div className="mx-auto max-w-7xl">
            <div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
              <div><SectionLabel testId="resources-eyebrow">{t.resources}</SectionLabel><h2 className="font-heading text-4xl" data-testid="resources-title">Useful context for the road ahead.</h2><p className="mt-4 max-w-xl text-sm leading-7 text-slate-500" data-testid="resources-description">Search practical first-step guides. These resources are general information and do not replace advice on your specific facts.</p></div>
