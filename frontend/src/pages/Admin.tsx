...
interface Booking { id: string; full_name: string; mobile: string; email: string; mode: string; slot: string; issue_description: string; document_name?: string | null; status: string; created_at: string }
    ...
    <div className="min-h-screen bg-[#f8fafc] text-[#0f172a]" data-testid="admin-dashboard-page">
      ...
      <main className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-16">
        ...
        <section className="mt-12 border border-slate-200 bg-white" data-testid="admin-bookings-section">
          ...
          {bookingsQuery.isLoading ? <p className="p-8 text-sm text-slate-500" data-testid="admin-bookings-loading">Loading requests...</p> : bookings.length === 0 ? <div className="p-10 text-center"><CheckCircle2 className="mx-auto size-8 text-[#b45309]" /><p className="mt-4 font-heading text-2xl" data-testid="admin-bookings-empty-title">No requests yet</p><p className="mt-2 text-sm text-slate-500" data-testid="admin-bookings-empty-copy">New consultation bookings will appear here.</p></div> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left" data-testid="admin-bookings-table"><thead className="bg-[#f8fafc] text-xs uppercase tracking-[0.12em] text-slate-400"><tr><th className="px-6 py-4">Client</th><th className="px-6 py-4">Mode / slot</th><th className="px-6 py-4">Issue</th><th className="px-6 py-4">Status</th></tr></thead><tbody>{bookings.map((booking) => <tr className="border-t border-slate-100 align-top" key={booking.id} data-testid={`admin-booking-row-${booking.id}`}><td className="px-6 py-5"><p className="font-medium" data-testid={`admin-booking-client-${booking.id}`}>{booking.full_name}</p><p className="mt-1 text-xs text-slate-400" data-testid={`admin-booking-contact-${booking.id}`}>{booking.mobile} · {booking.email}</p></td><td className="px-6 py-5 text-sm text-slate-600"><p data-testid={`admin-booking-mode-${booking.id}`}>{booking.mode}</p><p className="mt-1 text-xs text-slate-400" data-testid={`admin-booking-slot-${booking.id}`}>{booking.slot}</p></td><td className="max-w-sm px-6 py-5 text-sm leading-6 text-slate-600" data-testid={`admin-booking-issue-${booking.id}`}>{booking.issue_description}</td><td className="px-6 py-5"><span className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.1em] text-[#92400e]" data-testid={`admin-booking-status-${booking.id}`}><span className="size-1.5 rounded-full bg-[#b45309]" />{booking.status}</span></td></tr>)}</tbody></table></div>}
        </section>
